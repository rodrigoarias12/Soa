...
int main(int argc, char * argv[]){
	...
	*/
	...
	/*Fin datos de Conexión*/
	/*Final de la portada*/
	if(recv(caller_socket,&njug,sizeof(int),0)<0){
		imprimirError(4);
	}else if(!njug){
		//FUI RECHAZADO POR EL SERVIDOR
		//Imágen de Fui rechazadoquit
	...
	while(torneoVivo){
		...
		while(jugando){
			...
			printf("jugando %d, brun %d\n", jugando, bRun);
			while(jugando && bRun){	enviar=0;
...
void dibujarTecho(SDL_Surface *screen){
	...
	if(njug==1){ //Jugador 1
		...
		if(miPaquete.jugadores[0].puntos >= miPaquete.jugadores[1].puntos){ //Gané
		...
		if(miPaquete.jugadores[1].puntos > miPaquete.jugadores[0].puntos){ //Gané
			resultado = inicializarSprite(SPRITES_GANADO);
			SDL_mutexP(mtx);
			dibujarSprite(resultado,0,0,screen);
			SDL_Flip(screen);
	...
	//fin de todo
	printf("Puntajes Jugador N 1: %d", miPaquete.jugadores[0].puntos);
	printf("Puntajes Jugador N 2: %d", miPaquete.jugadores[1].puntos);
	sleep(60);
	//finalizar();
}
...
void mostrarInformacion(SDL_Surface *screen) {
	...
	if(njug==1)
	{
		texto2 = TTF_RenderText_Solid(fuente2,cadena,color);		
