...
void dibujarTecho(SDL_Surface *screen){
	...
	if(njug==1){ //Jugador 1
		...
		if(miPaquete.jugadores[0].puntos > miPaquete.jugadores[1].puntos){ //Gane
			...
			resultado = inicializarSprite(SPRITES_EMPATADO);
			SDL_mutexP(mtx);
			dibujarSprite(resultado,0,0,screen);
			SDL_Flip(screen);
			SDL_mutexV(mtx);
		...
		if(miPaquete.jugadores[1].puntos > miPaquete.jugadores[0].puntos){ //Gane
			...
			SDL_Flip(screen);
			SDL_mutexV(mtx);
		}else if(miPaquete.jugadores[1].puntos < miPaquete.jugadores[0].puntos){//Perdi
			resultado = inicializarSprite(SPRITES_PERDIDO);
			SDL_mutexP(mtx);
			dibujarSprite(resultado,0,0,screen);
			SDL_Flip(screen);
...
void finalizarTorneo(SDL_Surface *screen){
	printf("CLIENTE: Fin de Torneo Recibido\n");
	SDL_Surface *finTorneo = inicializarSprite(SPRITES_FIN_TORNEO);
	dibujarSprite(finTorneo,0,0,screen);
	SDL_mutexP(mtx);
	SDL_Flip(screen);
	SDL_mutexV(mtx);
