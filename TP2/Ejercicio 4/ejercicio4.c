...
// VALIDAR SI ES O NO RECURSIVO
    ...
    {
        printf("es un arhivo solo AHHHHHHHHHHHA\n ");
        int x;
        //funcion simpre sin el -r
        for( x=2; x<argc; x++)
        {
            ...
            if(!fork())
            {
                printf("ni ahi que entramos aca");
                norecur(argc,argv,x);
                return 0;
            }//fin if fork
        }//fin for archivos
...
int norecur(int argc, char *argv[],int x)
{
    ...
    printf("Empieza la corrida!!");
    while(fgets(linea,sizeof(linea),fp))
    {
        printf("Entra como un campeón");
        if(  match(&exp,linea))
        {
            printf("Matchea el pibe");
            if(!numeroaciertos)
            {
                printf("Le acierta");
                char *nombrearchivo;
                ...
                printf("\n\nNL %d LINEA %s ARGV[x] %s \n\n",nl,linea,argv[x]);
                fprintf(fps,"%s",argv[x]);
                fprintf(fps,"Linea %d : %s",nl,linea);
                numeroaciertos++;
            } else {
                fprintf(fps,"Linea %d : %s",nl,linea);
                numeroaciertos++;
