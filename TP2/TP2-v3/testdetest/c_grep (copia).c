...
int x=0;
   ...
   strncpy(extencion , strrchr(argv[2],'.')+1, end);         
   printf("EXTENCION : %s\n",extencion); 
printf("con contar archivo \n");     
   num=cuentaArchivos(path, 1,extencion,argv[1]);
   printf("conto archivos \n");
  char linea[50000];
...
unsigned cuentaArchivos(char *ruta, int niv,char *extencion,char*patron)
{
      ...
      if (tipo!=DT_DIR)
        ...
        { posstr=generaPosStr(niv);
         // printf("Entrando en: %s\n",nombrecompleto);          
          numfiles+=cuentaArchivos(nombrecompleto, niv+1,extencion,patron);
